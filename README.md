...
아무 신호가 오지 않습니다. 실기로 확인한 결과(2026-09-16) 프레임이 보내는 메시지는 화면이 열리고
닫히는 **라이프사이클 신호뿐**이고, 입력에 대응하는 신호는 **하나도 없습니다**.

그래서 되돌리기 타이머는 **마지막으로 관측된 활동**부터 셉니다. 관측되는 활동은 세 가지입니다.

- 키오스크 페이지 영역(머리글·덮개)의 터치·키 입력
- 포커스가 작성 프레임 **안으로 들어간 순간**(1회)
- 이폼사인이 보내는 라이프사이클 신호

되돌리기 기준은 두 가지로 나뉩니다.

- `idleResetSeconds` — 아직 아무도 손대지 않은 **빈 화면** 기준
- `abandonResetSeconds` — 작성을 시작한 방문자가 **자리를 뜬** 경우 기준

되돌리기 직전에는 `countdownSeconds` 초 동안 「계속 작성하시겠습니까?」가 뜨고, 그동안 작성 내용은
흐리게 가려집니다. 이 덮개는 화면 전체를 덮으므로 **아무 곳이나 터치하면 취소되고 타이머가 다시
시작**합니다.

> 🔴 **오래 쓰는 방문자는 카운트다운을 한 번 터치해 계속 씁니다.**
> 프레임 안의 입력을 볼 수 없으므로, 작성 중이라도 `abandonResetSeconds` 가 지나면 덮개가 뜹니다.
> 이것이 「글을 쓰다 그대로 떠난 방문자의 화면이 영원히 남는 것」을 막는 대가입니다.
> 서식이 길다면 `abandonResetSeconds` 를 넉넉히(예: `300`) 두세요.
> (2026-09-16 이전 판은 포커스가 프레임 안에 있는 동안 타이머를 멈춰, 작성 중 이탈에서 되돌리기가
> 영원히 오지 않는 결함이 있었습니다.)
